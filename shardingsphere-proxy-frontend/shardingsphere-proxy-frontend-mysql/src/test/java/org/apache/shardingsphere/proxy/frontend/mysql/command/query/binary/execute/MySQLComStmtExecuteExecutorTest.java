...
public final class MySQLComStmtExecuteExecutorTest {
    ...
    @Test
    public void t1() throws NoSuchFieldException, SQLException {
        when(backendConnection.getSchemaName()).thenReturn("logic_db");
        when(backendConnection.getDefaultSchemaName()).thenReturn("logic_db");
        MySQLComStmtExecutePacket packet = mock(MySQLComStmtExecutePacket.class);
        ...
        when(packet.getSql()).thenReturn(" select * from wide_order where wide_order . id  + wide_order.id = 2 and abs(id)=1 order by type limit 1;");
        ...
        when(packet.getSql()).thenReturn(" select wide_order.id, wide_order.name from wide_order where wide_order . id  + wide_order.id = 2 and name='1' limit 1;");
        when(packet.getSql()).thenReturn(" select * from wide_order where wide_order . id  + wide_order.id = 2 and abs(id)=1 limit 1;");
        when(packet.getSql()).thenReturn(" select wide_order.name, wide_order.id, wide_order.type from wide_order where wide_order . id  + wide_order.id > 1 limit 2;");
        when(packet.getSql()).thenReturn(" show tables");
        when(packet.getSql()).thenReturn(" desc wide_order");
        MySQLComStmtExecuteExecutor mysqlComStmtExecuteExecutor = new MySQLComStmtExecuteExecutor(packet, backendConnection);
        FieldSetter.setField(mysqlComStmtExecuteExecutor, MySQLComStmtExecuteExecutor.class.getDeclaredField("databaseCommunicationEngine"), databaseCommunicationEngine);
        when(databaseCommunicationEngine.execute()).thenReturn(new QueryResponseHeader(Collections.singletonList(mock(QueryHeader.class))));
        mysqlComStmtExecuteExecutor.execute();
